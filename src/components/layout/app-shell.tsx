 ...
 *
 * This is only for Phase 2 UI scaffolding.
 * In the authentication phase, this will be replaced by a secure server-side session.
 */
const DEVELOPMENT_PREVIEW_ROLE: UserRole = "CLIENT";
...
function SidebarNavigation(props: {
  ...
  return (
    <aside className="maurie-glass hidden lg:fixed lg:inset-y-0 lg:left-0 lg:z-30 lg:flex lg:w-80 lg:flex-col lg:px-5 lg:py-6">
      ...
      <nav aria-label="Primary navigation" className="mt-8 flex flex-1 flex-col gap-2">
      ...
      <div className="maurie-glass-soft mt-auto rounded-3xl p-4 text-xs leading-5 text-[var(--maurie-muted)]">
        Role-aware interface preview is active. Server-side middleware and database ownership checks
        will become the final security authority.
      </div>
    </aside>
  );
}
  ...
  return (
    <header className="maurie-glass sticky top-0 z-20 border-x-0 border-t-0">
      <div className="flex h-20 items-center justify-between px-4 sm:px-6 lg:px-10">
        ...
        <div className="rounded-full border border-black/10 bg-white/60 px-3 py-1 text-xs font-medium text-zinc-700 shadow-sm backdrop-blur-xl dark:border-white/10 dark:bg-white/10 dark:text-zinc-300">
          {ROLE_LABELS[props.role]}
        </div>
      </div>
    </header>
      ...
      <div className="grid grid-cols-4 gap-1">
        {props.navigationItems.slice(0, 4).map((item: NavigationItem) => (
          <Link
            key={item.href}
            href={item.href}
            className="rounded-3xl px-2 py-3 text-center text-[0.7rem] font-medium text-zinc-700 transition duration-300 ease-out hover:bg-black/5 dark:text-zinc-300 dark:hover:bg-white/10"
          >
            {item.label}
          </Link>
        ))}
...
export function AppShell(props: AppShellProps) {
  const role: UserRole = DEVELOPMENT_PREVIEW_ROLE;
  const navigationItems: readonly NavigationItem[] = getVisibleNavigation(role);

  return (
    <div className="min-h-dvh">
      <SidebarNavigation role={role} navigationItems={navigationItems} />
